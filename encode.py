...
def main():
    ...
    use_mmap = True if data_args.query_file.endswith('.jsonl') or data_args.corpus_file.endswith('.jsonl') else False
    if data_args.encode_is_qry:
        queries = GenericDataLoader(data_args.train_dir, corpus_file=data_args.corpus_file,
                                    query_file=data_args.query_file, use_mmap=use_mmap).load_queries()
        start, end = 0, len(queries)

    else:
        corpus = GenericDataLoader(data_args.train_dir, corpus_file=data_args.corpus_file,
                                   query_file=data_args.query_file, use_mmap=use_mmap).load_corpus()
    ...
    pretokenized = True if data_args.query_file.endswith('.jsonl') or data_args.corpus_file.endswith('.jsonl') else False
    encode_dataset = EncodeDataset(queries if data_args.encode_is_qry else corpus, tokenizer,
                                   ...
                                   start=start, end=end, pretokenized=pretokenized)
